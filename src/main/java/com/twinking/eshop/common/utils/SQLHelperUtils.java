...
import com.twinking.eshop.common.dialect.Dialect;
...
public class SQLHelperUtils {
    ...
    public static int getCount(final String sql, final Connection connection,
        ...
        try {
			...
			if (ReflectionsUtils.getFieldValue(boundSql, "metaParameters") != null) {
				...
				ReflectionsUtils.setFieldValue(countBS, "metaParameters", mo);
			}
			//解决MyBatis 分页foreach 参数失效 end 
            SQLHelperUtils.setParameters(ps, mappedStatement, countBS, parameterObject);
            rs = ps.executeQuery();
            int count = 0;
            if (rs.next()) {
                count = rs.getInt(1);
	...
	private static String removeOrders(String qlString) {  
        Matcher m = pattern.matcher(qlString);
